...
// 新しいCreateメソッド（SessionOptionsを受け取るバージョン）
std::unique_ptr<OnlineRecognizerImpl> OnlineRecognizerImpl::Create(
  ...
  if (!config.model_config.transducer.encoder.empty()) {
    ...
    auto decoder_model = ReadFile(config.model_config.transducer.decoder);
    auto sess = std::make_unique<Ort::Session>(
        env, decoder_model.data(), decoder_model.size(), session_options);

    size_t node_count = sess->GetOutputCount();

    if (node_count == 1) {
      return std::make_unique<OnlineRecognizerTransducerImpl>(config, session_options);
    } else {
      return std::make_unique<OnlineRecognizerTransducerNeMoImpl>(config, session_options);
    }
  }

  if (!config.model_config.paraformer.encoder.empty()) {
    return std::make_unique<OnlineRecognizerParaformerImpl>(config, session_options);
  }

  if (!config.model_config.wenet_ctc.model.empty() ||
      !config.model_config.zipformer2_ctc.model.empty() ||
      !config.model_config.nemo_ctc.model.empty()) {
    return std::make_unique<OnlineRecognizerCtcImpl>(config, session_options);
  }
...
// 新しいCreateメソッド（SessionOptionsを受け取るバージョン）
std::unique_ptr<OnlineRecognizerImpl> OnlineRecognizerImpl::Create(
    AAssetManager *mgr, const OnlineRecognizerConfig &config, Ort::SessionOptions &session_options) {
  if (!config.model_config.transducer.encoder.empty()) {
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING);
    ...
    if (node_count == 1) {
      return std::make_unique<OnlineRecognizerTransducerImpl>(mgr, config, session_options);
    } else {
      return std::make_unique<OnlineRecognizerTransducerNeMoImpl>(mgr, config, session_options);
    }
  }

  if (!config.model_config.paraformer.encoder.empty()) {
    return std::make_unique<OnlineRecognizerParaformerImpl>(mgr, config, session_options);
  }

  if (!config.model_config.wenet_ctc.model.empty() ||
      !config.model_config.zipformer2_ctc.model.empty() ||
      !config.model_config.nemo_ctc.model.empty()) {
    return std::make_unique<OnlineRecognizerCtcImpl>(mgr, config, session_options);
  }
...
// 新しいコンストラクタ
OnlineRecognizerImpl::OnlineRecognizerImpl(const OnlineRecognizerConfig &config, Ort::SessionOptions* session_options)
    : config_(config) {
  // オプションに応じた実装を追加
  if (!config.rule_fsts.empty()) {
    std::vector<std::string> files;
...
// 新しいコンストラクタ（SessionOptionsを受け取るバージョン）
OnlineRecognizerImpl::OnlineRecognizerImpl(AAssetManager *mgr, const OnlineRecognizerConfig &config, Ort::SessionOptions &session_options)
    : config_(config) {
  // オプションに応じた実装を追加
  if (!config.rule_fsts.empty()) {
    std::vector<std::string> files;
